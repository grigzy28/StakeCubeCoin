...
bool CBlockHeader::IsProgPow() const {
    ...
    return (nTime > SCC_GEN_TIME && nTime >= CChainParams::Params().GetConsensus().nPPSwitchTime);
}

bool CBlockHeader::IsFirstProgPow() const {
    return (IsProgPow() && nTime <= (CChainParams::Params().GetConsensus().nPPSwitchTime + 432000)); //5 days
}
